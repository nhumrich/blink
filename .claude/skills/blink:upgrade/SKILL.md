...
- **Current version**: Check `blink.toml` for a `blink-version` field under `[package]`. If not found, fall back to `blink.lock` for a `blink-version` field under `[metadata]`. If neither exists, ask the user what version they are upgrading from.
...
Finally, update the `blink-version` field in `blink.toml` under `[package]` to match the target version. If `blink.lock` also exists, update its `blink-version` field as well.
